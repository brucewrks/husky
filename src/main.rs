...
fn main() {
    ...
    // Sanity checks on known mates
    {
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 500, false);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 1000, true);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 2000, false);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 2000, true);
        go_assert(best_move, "a4d7", position);
    }
