    ...
    // Sanity checks on known mates
    {
        ...
        fn go_assert(mov:ChessMove, correct:&str, fen:&str) {
            if format!("{}", mov) != correct {
                panic!("Got bad output for a engine sanity check. Expected: {} Got: {}. Position: {}", correct, mov, fen);
            }
        }
        ...
        let (mut best_move, mut _best_eval) = evaluator.best_move(board, 100, false);
        ...
        (best_move, _best_eval) = evaluator.best_move(board, 100, false);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 15000, false);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 35000, true);
        go_assert(best_move, "f8c5", position);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 65000, false);
        ...
        board = Board::from_str(position).unwrap();
        (best_move, _best_eval) = evaluator.best_move(board, 65000, true);
        go_assert(best_move, "a4d7", position);
    }
